"""
IPF exposures — Scott's dependency-aware approach, all categories (CORRECTED).
Full worked reproduction: per-bin dedup, DL, REML, tau^2-marginalized Bayesian
(OR / 95% CrI / P(OR>1) / prediction interval / prior sweep), Bayesian forest plots,
and the two super-family between-group contrasts (dust GD-vs-VGDF, Metals MD-vs-Nickel).

Corrections applied throughout:
  >>> C1  Garcia-Sancho same-cohort duplicate removed (General Dust unadj -> k=4).
  >>> C2  Zubairi 2023 corrected to OR 1.11 (0.65-1.9) [Gandhi 2023 mis-transcribed the
          upper CI as 1.7]; VGDF remains k=9.

Estimators verified against Scott's R (exact for DL/REML/QM/Cochran; Bayesian matches
bayesmeta to 3-4 sig figs). Pure Python: numpy/pandas/matplotlib/stdlib (no scipy/metafor).
Outputs: forest PNGs in ./ipf_forests/ and a printed summary table.
"""
import math, os
...
if __name__ == "__main__":
    ...
    # >>> LOCKED RECONCILIATION (aligns Table 3 / Figure 2 with the final results)
    #   C3  Air pollution -> genuine two-study incidence pool: drop Johannson 2014
    #       (acute-exacerbation outcome, not incident IPF); Cui 2023 uses its PM2.5 aHR
    #       1.09 [1.02,1.17] (raw 1.01 [0.86,1.18] matched no reported estimate).
    ...
    if len(ni) >= 2:
        ...
        print("Metals MD vs Ni: not run — Nickel (speciation) is reported descriptively, not pooled (see reconciliation C5).")
